...
affiliations: "NVIDIA"
summary: "웹 비디오로 사전학습된 video diffusion 모델이 미래 영상과 action을 함께 생성하게 하면, action 학습은 모방에서 inverse dynamics로 바뀌고 반복 없는 이질적 데이터만으로 새 동작과 새 환경에 일반화한다."
paper_url: "https://arxiv.org/abs/2602.15922"
code_url: "https://github.com/dreamzero0/dreamzero"
---

> **핵심 주장** — VLA에게 부족한 것은 반복 시연의 양이 아니라 **"세계가 어떻게 움직이는가"에 대한 prior** 다. 웹 비디오로 사전학습된 video diffusion 모델이 미래 영상과 action을 함께 생성하게 하면, action 학습은 "상태에서 행동을 모방하는 문제"에서 "상상한 미래로부터 행동을 읽어내는 문제(inverse dynamics)"로 바뀐다. 그 결과 반복 없는 이질적 데이터만으로도 새 동작과 새 환경에 일반화한다.

---

## 들어가며

VLA(Vision-Language-Action) 모델의 표준 레시피는 사전학습된 VLM에 action head를 붙이고 로봇 시연으로 학습하는 것이다. RT-2, OpenVLA, π0.5, GR00T가 모두 이 틀 안에 있다. VLM이 웹에서 배운 의미 지식을 로봇으로 옮겨오는 전략이다.

DreamZero는 출발점을 바꾼다. VLM 대신 **image-to-video diffusion 모델**(Wan2.1-I2V-14B)을 백본으로 삼고, 미래 영상과 action을 하나의 모델에서 함께 생성한다. 논문은 이 부류를 **World Action Model(WAM)** 이라 부른다. 이 글은 논문의 논지를 따라가되, 로보틱스 배경이 얕은 독자도 각 설계 결정의 **이유**를 납득할 수 있도록 필요한 개념을 그때그때 쌓아가며 진행한다.
...
VLM은 정적인 image-text 쌍으로 학습된다. 그래서 의미 수준의 지식은 풍부하지만, 물체가 어떻게 움직이고 접촉하면 무엇이 변하는지에 대한 **시공간적(spatiotemporal) prior** 는 비어 있다. 논문의 예시가 이 차이를 선명하게 보여준다.
...
VLM prior는 *what* 을 인코딩하지만, 기하·동역학·모터 제어와 정렬된 *how* 는 인코딩하지 않는다.

### 1.2 그 공백을 데이터로 메우는 대가

논문의 related work에 따르면 기존 VLA는 이 공백을 데이터 수집으로 메운다.

- **⓵ 환경 일반화** — 특정 태스크를 수백 개 환경에서 teleoperation으로 수집한다 (π0.5).
- **⓶ 태스크 일반화** — 언어 조건부 motion primitive 라이브러리를 키운다 (Gemini Robotics). 그러나 가능한 물리 상호작용 전체를 episode 단위 태스크 목록으로 덮는 것은 본질적으로 비현실적이다.

이 전략은 데이터 수집 방식까지 규정한다. VLA는 **같은 태스크를 비슷한 조건에서 반복한 구조화된 시연**에 의존한다. 논문의 가설은 이렇다. action만 예측하는 모델은 noisy한 state–action 쌍에서 동역학을 암묵적으로 추론해야 하므로, 반복 없는 이질적 데이터로는 잘 학습하지 못한다. 반면 video world model은 데이터의 **모든 연속 프레임 쌍**에서 학습 신호를 얻고, 웹 스케일 비디오로 물리 동역학을 이미 익혀 둔 상태다.

두 접근의 정보 흐름을 나란히 놓으면 이렇다.
...
- VLA는 관측에서 action으로 곧장 간다
- WAM은 미래 영상을 함께 그리고, action은 그 미래와 정렬되도록 생성된다
...
**⛔ 벽 2 — 아키텍처 선택.** bidirectional(BD)과 autoregressive(AR) 중 무엇이 맞는지 불분명하다. modality 정렬, 오차 누적, 추론 효율이 모두 이 선택에 걸려 있다.
...
로봇 학습에서 "모델"은 무엇을 조건으로 무엇을 예측하느냐에 따라 세 종류로 나뉜다.
...
DreamZero의 핵심 문장은 "action 학습을 dense state–action 모방에서 inverse dynamics로 옮긴다"이다. 그런데 policy든 IDM이든 결국 action을 출력한다. **inverse dynamics로 바꾸는 것이 왜 문제를 쉽게 만드는가?**

> ### 💡 VLA는 "미래를 적분한 평균"을 배우고, WAM은 그 적분을 풀어서 배운다
>
> 정책은 가능한 미래 관측 $o'$에 대해 다음처럼 분해할 수 있다.
>
...
>
> **VLA는 좌변을 직접 학습한다.** 같은 $(o,c)$에 유효한 전략이 여럿이면(왼손/오른손, 위에서/옆에서 잡기) 그 다봉성이 전부 action 분포로 접혀 들어온다. 모드마다 반복 시연이 필요해지는 이유다.
>
> **WAM은 두 인자를 나눠 학습한다.** 다봉성은 $p(o'\mid o,c)$가 떠안는데, 이 부분은 웹 비디오 사전학습이 이미 잘한다. 미래가 주어진 뒤의 $p(a\mid o,o')$는 거의 결정적이라 엔트로피가 낮다.
>
...
비디오 diffusion 모델이 영상을 만드는 방식은 두 가지다.
...
DreamZero는 flow matching으로 학습한다. 이 논문의 표기는 다음과 같다.
...
- 입력은 관측 영상, 언어, proprioception, 그리고 denoise할 noisy action이다
- 단일 AR DiT가 chunk 단위로 미래 영상 latent와 action을 함께 denoise한다
- 실행 후에는 실제 관측을 KV cache에 넣고 다음 chunk로 넘어간다
...
- 원문은 이 분포를 $\pi_0$로 표기하지만, π0 모델과 헷갈리지 않도록 여기서는 $\pi_\theta$로 쓴다

**벽 1에 대한 답은 이 분해를 두 모델로 구현하지 않는 것이다.** 별도의 video model과 IDM을 두는 대신(Li et al., 2026; Pai et al., 2025의 방식), 하나의 모델이 [영상, action]을 공유 objective로 joint denoise한다. 정렬을 두 head의 사후 결합으로 맞추지 않고, 같은 attention 안에서 함께 생성해 확보한다는 발상이다. 다만 논문의 근거는 "We believe"다. 분리형과 비교한 ablation은 없다.
...
>
> chain rule만 적용하면 두 번째 항은 다음이 된다.
>
...
> | 경우 | 가정의 성립 |
> |---|---|
> | 영상이 동작을 시각적으로 특정한다 (대부분의 pick-and-place) | 성립 |
> | 같은 시각 궤적인데 힘 조절이 다르다 ("살살 눌러") | 깨질 수 있음 |
> | 접촉 지점이 가려져 영상이 동작을 과소결정한다 | 깨질 수 있음 |
>
> 단, 실제 모델은 두 항을 따로 학습하지 않으므로 action token도 attention으로 $c$를 본다. 식 (1)은 구조적 제약이라기보다 **설계의 정당화**에 가깝다.

### 3.2 AR chunk-wise 학습 — 벽 2를 넘기

논문이 드는 AR의 이점은 세 가지다. KV cache로 추론이 빠르고, 관측 이력을 다음 생성의 guidance로 쓸 수 있으며, BD의 modality 정렬 문제를 피한다. 세 번째가 결정적인데, 부록 B의 예시로 보면 이렇다.

긴 demo의 특정 구간에 "put the black objects into the drawer"라는 캡션이 붙어 있다고 하자.
...
BD는 "언어 정렬"과 "시간 해상도" 중 하나를 포기해야 하는 딜레마에 빠진다. AR은 context를 받는다는 구조 덕분에 이 딜레마 자체를 피한다.

chunk 규격(부록 C, AgiBot 기준)은 다음과 같다.
...
- video 5 FPS, action 30Hz. chunk당 latent frame $K=2$, action horizon $H=48$이라 chunk당 1.6초다
- $K=2$가 $K=1$보다 나았다고 보고한다
- DROID는 action 15Hz, $H=24$로 역시 chunk당 1.6초다
- 기본 chunk 수는 4개이고, 최대 visual context는 6.6초다

한 가지 설계가 더 있다. **AR은 video modality에만 적용한다.** 논문이 밝힌 이유는 closed-loop action 예측에서 오는 오차 전파를 피하기 위해서다.

학습 objective는 chunk별 flow matching이다. 먼저 chunk $k$의 noisy 입력을 만든다.
...
- $t_k$ — chunk $k$의 timestep. **chunk 안에서는 video와 action이 공유하고, chunk끼리는 독립**이다

그리고 두 modality의 velocity를 함께 회귀한다.
...
학습은 trajectory 단위로 한 번에 하고, attention mask로 현재 noisy chunk가 이전 clean chunk만 보게 한다. LLM이 가변 길이 시퀀스를 causal mask로 한 번에 학습하는 것과 같은 방식이다.

> ### ⚠️ 팩트체크 — 학습 objective 주변의 표기 비일관
>
> 세 곳이 서로 맞지 않는다.
>
> - **context에 action이 들어가는가.** 식 (3)과 Algorithm 1은 $\mathcal{C}_k$를 과거 chunk의 clean 영상 latent와 clean action $a^j_1$의 쌍으로 정의한다. 그러나 본문은 "AR은 video에만"이라고 하고, Fig. 14와 Algorithm 2의 추론 context도 영상뿐이다. 본문과 Fig. 14 쪽이 의도로 보인다.
> - **$K$의 이중 사용.** 부록 C에서 $K$는 chunk당 latent frame 수(=2)인데, 식 (3)에서는 chunk에 대한 합의 범위로 쓰인다. Algorithm 1은 같은 자리에 $M$을 쓴다.
> - **timestep 규약.** Algorithm 2는 clean GT를 주입하는 호출에 $t=0$을 붙이는데, 식 (2)의 규약에서 $t=0$은 순수 노이즈다.
...
- **⓵ 생성** — 노이즈에서 출발해 [영상, action]을 joint denoise한다
...
순진한 구현이 chunk당 5.7초 걸리는 원인은 셋이다.
...
**스택 전체의 최적화.** 그 위에 최적화를 누적한다 (Table 1, H100 baseline = 1×).
...
>
> 1-step forward에서 입력된 영상 token 자체에는 픽셀 정보가 없다. 하지만 DiT 내부에서 그 token의 hidden state는 "이 노이즈를 어떤 미래로 보낼지", 즉 clean latent를 향한 velocity를 계산하고 있다. action token은 attention으로 이 hidden state를 본다. **계획이 denoise된 영상이 아니라 feature로 존재하는 것이다.**
>
> 이렇게 보면 Flash는 명시적 visual planning에서 "과거의 clean context + 암묵적 미래"로 action을 뽑는 쪽으로 한 걸음 이동한 셈이다. 4-step 83%가 1-step Flash 74%로 내려가는 것은 이 해석과 부합한다. 식 (1)의 IDM 해석이 가장 문자 그대로 성립하는 것은 multi-step 추론일 때다.
>
> 논문에 없는 해석이며, 내부 표현을 분석한 실험으로 뒷받침된 것은 아니다.

마지막으로 **action chunk smoothing**을 건다. action chunk를 cubic interpolation으로 2배 upsample하고, Savitzky-Golay filter(window 21, 3차)로 고주파 노이즈를 누른 뒤, 원래 해상도로 downsample한다.

---

## 4. 왜 작동하는가

논문의 자기설명은 네 가지로 정리된다.
...
⓶의 사례가 구체적이다. "마커를 집어 화이트보드에 선을 그어라"에서 모델은 마커를 반대 손에 건네는 영상을 그렸고, 로봇도 그대로 건넸다. "오븐에 크루아상을 구워라"에서는 오븐을 열기 전에 빵부터 집는 영상을 그렸고, 로봇은 빵을 든 채 오븐 앞에서 멈췄다. 논문은 여기서 "로봇 능력의 개선은 비디오 생성 능력의 개선으로 환원된다"고 결론 내린다.
...
AgiBot G1 결과(평균 task progress, %)다.
...
- scratch VLA는 같은 이질적 데이터에서 거의 0%다. 쉬운 pick-and-place에서도 올바른 물체 쪽으로 손을 뻗을 뿐 제대로 상호작용하지 못한다
- unseen 10개 태스크 중 강한 것은 "Remove Hat from Mannequin" 85.7, "Shake Hands" 59.2다
- post-training은 셔츠 개기 동률(92.5 vs 92.5), 과일 포장 큰 우위(96 vs 71), 테이블 정리 소폭 우위(83 vs 76)다. unseen 환경에서 평가하므로, 환경 일반화가 post-training 후에도 유지된다는 근거가 된다
- 정성적으로, pretrained VLA는 지시와 무관하게 물체로 손을 뻗어 잡으려는 경향을 보였다. 지배적 학습 행동(pick-and-place)에 과적합했다는 해석이다

DROID-Franka의 unseen 태스크(DROID에 없는 동사 20개)에서는 task progress 49 / 31 / 33, 성공률 22.5 / 12.5 / 7.5다(DreamZero / GR00T N1.6 / π0.5 순).

> ### ⚠️ 팩트체크 — "over 2×"는 AgiBot 기준이고, 지표는 부분 점수다
>
> abstract는 실로봇 실험에서 VLA 대비 2배 이상의 일반화 향상을 말한다. AgiBot에서는 seen 2.27×(62.2/27.4), unseen 2.42×(39.5/16.3)로 주장과 일치한다. 그러나 DROID unseen의 task progress는 49/33 ≈ 1.5×다. "2배 이상"은 AgiBot 결과에 대한 서술로 읽어야 한다.
>
> 또 주요 수치가 모두 task progress라서 완수율과는 다르다. DROID에서 DreamZero의 task progress 49와 성공률 22.5가 보여주듯, 둘은 크게 벌어질 수 있다.
...
- 향상 폭이 가장 큰 것은 Robot2Robot이다. 논문은 YAM과 AgiBot이 둘 다 bimanual parallel gripper라 embodiment gap이 좁기 때문으로 본다
- 형태 차이가 크고 시점이 흔들리는 사람 영상도 거의 같은 폭으로 향상시켰다

> ### ⚠️ 팩트체크 — 이 실험이 보여주는 것의 범위
>
> - **전이 영상이 평가 태스크 그 자체다.** 9개 unseen 태스크를 태스크당 8개씩 시연한 72개 trajectory다. "action 데이터 기준으로 unseen인 태스크를 영상으로 보여줬을 때"의 효과이지, zero-shot 전이가 아니다.
> - **추가 학습량의 대조군이 없다.** baseline은 추가 10K step을 거치지 않은 체크포인트다. 사전학습 데이터만으로 10K step을 더 돌린 대조군은 없다.
> - **"over 42%"의 정확한 값.** 상대 향상은 Robot2Robot +44.6%, Human2Robot +41.8%다.
...
모든 ablation은 계산 제약 때문에 50K step, batch 32로 학습하고 PnP Easy 태스크에서만 평가했다(Table 4).
...
- **다양성** — 같은 시간이라도 다양한 데이터가 낫다. 쉬운 pick-and-place에서도 그렇다
- **크기** — VLA는 크기를 키워도 이질적 데이터를 소화하지 못하고 물체 근처에서 맴돌기만 한다. 모델 용량만으로는 해결되지 않는다는 뜻이다
- **BD vs AR** — task progress는 같지만 AR 쪽 motion이 눈에 띄게 부드럽고, KV cache 덕에 추론이 3–4배 빠르다
...
같은 WAM 계열 안에서 논문이 스스로를 구분하는 축은 다음과 같다.
...
"world model"이라는 이름은 WAM 말고도 여러 갈래에 쓰인다. 그렇다면 V-JEPA 2나 Dreamer 같은 world model과 WAM은 무엇이 다른가?
...
>
> forward model은 "이 action을 하면 어떻게 되나"에 답하므로, 좋은 action을 찾으려면 추론 시 탐색이 필요하다. WAM은 action을 미래와 함께 생성하므로 탐색이 없고, 7Hz 제어가 가능한 것도 이 덕분이다. 대신 WAM은 여러 action 후보를 가정해 결과를 비교하는 식의 planning은 하지 않는다.
...
DreamZero의 진짜 기여는 특정 트릭이 아니다. **로봇 정책의 대부분을 비디오 생성 모델에 맡기고, 로봇 전용으로 배우는 부분을 얇은 action 판독으로 줄여도 된다**는 것을 실로봇에서 보인 점이다. 그러면 로봇 데이터의 역할도 바뀐다. 같은 태스크를 반복해 모드를 채우는 것이 아니라, 다양한 상황의 프레임 쌍으로 IDM을 넓히는 것이 된다.

그리고 이 논문은 LLM·diffusion 배경을 가진 사람에게 유난히 잘 읽힌다.

| 배경 | DreamZero에서 보이는 대응물 |
|---|---|
| Diffusion | action은 영상에 동기화된 **또 하나의 트랙**이다. 영상-오디오 joint 생성에서 오디오 자리에 action이 들어간 구조다. Flash는 few-step 추론에 맞춰 학습 timestep 분포를 설계하는 익숙한 기법을 modality 간 불일치로 확장한 것이다 |
| LLM | teacher forcing, causal mask, KV cache를 그대로 가져왔다. 다른 점은 추론 시에도 환경이 GT를 공급해 exposure bias를 구조적으로 없앴다는 것이다 |
| Agentic AI | DreamZero는 순수 System 1이다. 다단계 절차가 필요한 과제에는 planner–executor 분리가 여전히 필요하며, 논문도 Hi Robot 같은 dual-system 구조를 보완책으로 꼽는다 |

마지막으로 데이터 전략에 대한 함의가 있다. "태스크당 시연 수보다 태스크 다양성"이라는 명제는 이 논문에서 architecture prior와 **결합될 때만** 성립했다. 같은 데이터로 scratch VLA는 거의 아무것도 배우지 못했다. 데이터 수집 방식과 모델 구조는 따로 평가할 수 없는 한 쌍이다.
...
| 용어 | 정의 |
|---|---|
| **WAM (World Action Model)** | 미래 world state와 action을 정렬된 방식으로 함께 예측하는 foundation model. 영상 외에 촉각·힘 등으로 확장될 수 있다는 뜻에서 "Video Action Model" 대신 이 이름을 쓴다 |
...
| **GT 관측 주입** | 추론 시 예측 영상 대신 실제 카메라 관측을 KV cache에 쓰는 것. 학습 조건과 추론 조건을 일치시킨다 |
