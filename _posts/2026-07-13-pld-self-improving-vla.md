...
그런데 여기서 자연스러운 반문이 생깁니다. LLM의 RL도 결국 희소 보상을 쓰지 않을까요? GRPO는 최종 정답 여부만 보는 결과 보상(outcome reward)으로 학습합니다. 그것이 잘 작동하는데, 왜 로봇에서는 희소 보상이 벽이 될까요?
...
그런데 여기서 다시 반문이 생깁니다. LLM의 RL은 이런 문제를 겪지 않습니다. GRPO는 거대 모델도 문제없이 RL로 학습합니다. 무엇이 다를까요?
...
그렇다면 안정성은 어디서 올까요? 이 설계의 우아한 지점이 여기에 있습니다. **PLD는 안정화를 정책 손실에서 빼서 다른 세 곳으로 분산시켰습니다.**
